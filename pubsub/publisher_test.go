...
func TestPublisher(t *testing.T) {
	...
	t.Run("InvalidMessage", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, pubsub.ErrInvalidArgument, "expected ErrInvalidArgument due to nil message")
	})
	...
	t.Run("RequestEmptyMessage", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, pubsub.ErrInvalidArgument, "expected ErrInvalidArgument for nil message")
	})
...
func TestPublisherClose(t *testing.T) {
	...
	t.Run("Close", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, pubsub.ErrCloseConnection, "Expected error to be ErrCloseConnection when publishing with a closed Publisher")
	})
}
...
func FuzzPublisher(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, subject string, payload []byte) {
		...
		if subject == "" {
			...
			assert.ErrorIs(t, err, pubsub.ErrInvalidArgument, "Expected ErrInvalidArgument due to empty subject")
